<template>
    <div class="flex flex-col gap-6">
        <NuxtLink to="/vacancies"
                class="w-fit py-1.5 px-4 rounded-xl hover:text-white hover:bg-indigo-500 border border-indigo-500 transition-all duration-500 bg-transparent text-indigo-500">
                Назад</NuxtLink>
        ...
        <div class="flex flex-col gap-2 text-base text-gray-400">
            <p><span class="font-semibold">Опыт:</span> {{ vacancy?.exp }}</p>
            <p><span class="font-semibold">График:</span> {{ vacancy?.schedule }}</p>
        ...
        <div class="flex flex-col gap-4">
            <p class="text-2xl font-mono font-semibold">Описание вакансии</p>
            <p>{{ vacancy?.desc }}</p>
        </div>
        <div class="flex flex-col gap-4">
            <p class="text-2xl font-mono font-semibold">Требования вакансии</p>
        ...
        <button @click="sendResponse(vacancy?.id)"
            ...
            class="w-fit py-1.5 px-4 rounded-xl border border-indigo-500 bg-indigo-500 text-white transition-all duration-500">{{

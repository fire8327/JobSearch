        ...
        <FormKit @submit="addEntry" type="form" :actions="false" messages-class="hidden" form-class="flex flex-col gap-6 items-center justify-center">
            ...
            <template v-if="role === 'applicant'">
                ...
                <div class="flex items-center lg:items-start gap-4 max-lg:flex-col w-full md:w-2/3 lg:w-1/2">
                    ...
                    <FormKit v-model="mainForm.salary" validation="required|number" messages-class="text-[#E9556D] font-mono" type="text" placeholder="Желаемая зарплата" name="Желаемая зарплата" outer-class="w-full lg:w-1/2" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-indigo-500 shadow-md"/>
                </div>
                <FormKit v-model="mainForm.education" validation="required" messages-class="text-[#E9556D] font-mono" type="select" :options="['Среднее', 'Среднее специальное', 'Неоконченное высшее', 'Высшее', 'Бакалавр', 'Магистр', 'Аспирантура']" placeholder="Образование" name="Образование" outer-class="w-full md:w-2/3 lg:w-1/2" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-indigo-500 shadow-md"/>
                <FormKit validation="required" @change="(e) => { resumeFile = e.target.files[0] }" accept=".pdf, .doc, .docx" messages-class="text-[#E9556D] font-mono" type="file" label="Резюме" name="Резюме" outer-class="w-full md:w-2/3 lg:w-1/2" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-indigo-500 shadow-md"/>
                <FormKit v-model="skillsInput" validation="required" messages-class="text-[#E9556D] font-mono" type="textarea" placeholder="Напишите навыки через запятую" name="Навыки" outer-class="w-full md:w-2/3 lg:w-1/2" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-indigo-500 shadow-md"/>
                <div class="flex flex-col gap-2 w-full md:w-2/3 lg:w-1/2" v-if="mainForm.skills && mainForm.skills.length > 0">
                    ...
                    <div class="w-full flex items-center gap-2 flex-wrap">
                        <button type="button" @click="removeSkill(index)" class="cursor-pointer flex items-center gap-2 py-1.5 px-4 rounded-xl bg-indigo-500 text-white text-sm uppercase" v-for="(skill, index) in mainForm.skills" :key="index">
                            <span>{{ skill }}</span>
            ...
            <template v-if="role === 'employer'">
                ...
                <div class="flex items-center lg:items-start gap-4 max-lg:flex-col w-full md:w-2/3 lg:w-1/2">
                    ...
                    <FormKit v-model="mainForm.salary" validation="required|number" messages-class="text-[#E9556D] font-mono" type="text" placeholder="Зарплата" name="Зарплата" outer-class="w-full lg:w-1/3" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-indigo-500 shadow-md"/>
                    <FormKit v-model="mainForm.format" validation="required" messages-class="text-[#E9556D] font-mono" type="select" :options="['Удалённый', 'Офис', 'Гибрид', 'Вахта', 'Проектный']" placeholder="Формат" name="Формат" outer-class="w-full lg:w-1/3" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-indigo-500 shadow-md"/>
                </div>
                ...
                <FormKit v-model="mainForm.desc" validation="required" messages-class="text-[#E9556D] font-mono" type="textarea" placeholder="Описание вакансии" name="Описание вакансии" outer-class="w-full md:w-2/3 lg:w-1/2" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-indigo-500 shadow-md"/>
                <FormKit v-model="mainForm.requirements" validation="required" messages-class="text-[#E9556D] font-mono" type="textarea" placeholder="Требования" name="Требования" outer-class="w-full md:w-2/3 lg:w-1/2" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-indigo-500 shadow-md"/>
            </template>

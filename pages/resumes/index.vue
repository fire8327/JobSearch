<template>
        ...
        <div class="flex flex-col gap-6">
            ...
            <div class="flex flex-col items-center gap-6 w-full">
                ...
                <div class="flex items-center lg:items-start gap-4 max-lg:flex-col w-full md:w-2/3 lg:w-1/2">
                    ...
                    <FormKit v-model="searchQuery.education" type="select" :options="['Среднее', 'Среднее специальное', 'Неоконченное высшее', 'Высшее', 'Бакалавр', 'Магистр', 'Аспирантура']" placeholder="Образование" name="Образование" outer-class="w-full lg:w-1/2" input-class="focus:outline-none px-4 py-2 bg-white rounded-xl border border-transparent w-full transition-all duration-500 focus:border-indigo-500 shadow-md"/>
                </div>
            </div>
        </div>
    ...
    <template v-else>
        <div class="flex flex-col gap-6 items-center justify-center text-center">
            ...
            <NuxtLink to="/profile" class="mx-auto px-4 py-2 border border-indigo-500 text-indigo-500 rounded-full w-[160px] text-center transition-all duration-500 hover:text-white hover:bg-indigo-500">Продолжить</NuxtLink>
        </div>
    </template>
</template>
